...
Definition a := [["t";"_";"y";"#";"1"];["t";"_";"x";"#";"6"];["t";"_";"y";"#";"3"];["t";"_";"x";"#";"4"]].
...
Definition create_vartable (input : list LString.t) (card : nat) : list nat :=
  let s := input in
  let f := (fun (x : LString.t) => if "y" \in x then (extractNumber (drop 4 x)) + pow2_min1 card
                                else if "z" \in x then (2 * pow2_min1 card)
                                else (extractNumber (drop 4 x) )) in
  [seq f x | x <- s].

(*Compute toStringNat( create_vartable a 3).*)
...
(** parse the content of an LString into a sparse matrix. Create the correct chvatal-matrix and check if they are the same. Print some logging information *)
  Definition parse_content (content : LString.t) (card : nat)(fixes : LString.t) : C.t System.effect unit :=
    ...
    do! System.log (LString.s "Variable section:") in
        ...
        do! System.log (LString.s "Relevant input:") in
            ...
            do! System.log (LString.s "Ordered input:") in
                ...
                (*create check-matrix*)
                let ch_matrix := if eqb fixes (of_string("y"))
                                 then order(create_mat_rev card)
                                 else  order(create_mat card) in
...
Definition read_file (argv : list LString.t) : C.t System.effect unit :=
  do! System.log (LString.s "Enter file-name of VIPR certificate to be checked:") in
    ...
    do! System.log (LString.s "What is [n]?") in
        ...
        | None => ret tt
        | Some number =>
          do! System.log (LString.s "Is this an instance with fixings (y/n)?") in
              let! fixings := System.read_line in
              match fixings with
              | None => ret tt
              | Some fixes =>
                match content with
                | None => System.log (LString.s "Cannot read the file")
                | Some content => parse_content content (extractNumber  number) fixes
                end
              end
        end
    end.
